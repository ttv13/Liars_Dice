...
int main() {
    ...
    xil_printf("Memory Game Initialized\n");
    ResetGame();
...
void HandleKeyPress(char key) {
    xil_printf("Handling key: %c, GameState: %d\n", key, gameState);

    if (key == 'C') {
        xil_printf("Reset key pressed\n");
        ResetGame();
        return;
    }
...
void UpdateDisplay() {
    ...
    switch (gameState) {
        ...
        case 3:
            ...
            OLED_PutString(&oled, "Press C to restart"); // Corrected text here
            break;
    }
...
void GenerateSequence() {
    xil_printf("Generating new sequence\n");
...
void CheckUserInput() {
	...
	if (correct) {
		...
		// Input was incorrect
		xil_printf("User input incorrect\n");
